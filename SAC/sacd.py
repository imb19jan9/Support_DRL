...
class SACD(OffPolicyAlgorithm):
    ...
    def train(self, gradient_steps: int, batch_size: int = 64) -> None:
        ...
        for gradient_step in range(gradient_steps):
            ...
            current_q_esimates = self.critic(
                replay_data.observations, replay_data.actions.squeeze(-1)
            )

            # Compute critic loss
            critic_loss = 0.5 * sum(
                [F.mse_loss(current_q, q_backup) for current_q in current_q_esimates]
            )
    ...
    def excluded_save_params(self) -> List[str]:
        ...
        """
        # Exclude aliases
        return super(SACD, self).excluded_save_params() + [
            "actor",
            "critic",
            "critic_target",
        ]
